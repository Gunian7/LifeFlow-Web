...
import { aiPost, aiQuota, AiQuotaError } from '../ai-client'
...
export function AiCard({ tasks, apiBaseUrl, adopted, onAdopt, onRestoreRules }: AiCardProps) {
  ...
  useEffect(() => {
    let cancelled = false
    aiQuota(apiBaseUrl).then((snapshot) => {
      if (!cancelled && snapshot.mode === 'enforced') setRemaining(snapshot.remaining)
      else if (!cancelled) setRemaining(null)
    }).catch(() => { /* 额度未知时就不显示 */ })
  ...
  async function ask() {
    ...
    try {
      const result = await aiPost<{ ok: boolean; order: string[]; reason: string }>(apiBaseUrl, '/v1/ai/order', { tasks: candidates })
      setOrder(result.order); setReason(result.reason); setState('ready')
    } catch (error) {
      setState('error')
      setReason(error instanceof AiQuotaError ? '今日 AI 次数已用完，明天 0 点重置。' : 'AI 暂时不可用，本地计划没有改变。')
    }
  }
  ...
  return (
    <section className="edit-card ai-card" aria-label="AI 建议顺序">
      ...
      {state === 'idle' && <><p className="settings-copy">让 AI 根据重要性和今天剩下的时间给任务排个顺序。它只提建议，采纳与否由你决定。{remaining !== null && <span className="quota-left">今日剩余 {remaining} 次。</span>}</p><button className="secondary-button" type="button" onClick={ask}>查看建议顺序</button></>}
