...
function validateOrderTasks(body) {
  if (!body || typeof body !== "object") return null;
  const bodyTasks = body.tasks;
  if (!Array.isArray(bodyTasks) || bodyTasks.length === 0 || bodyTasks.length > 20 || !bodyTasks.every((item) => isTask(item))) return null;
  const ids = bodyTasks.map((task) => task.id);
  if (new Set(ids).size !== ids.length) return null;
  return bodyTasks.map((value) => ({ id: value.id.trim(), title: value.title.trim(), durationMinutes: value.durationMinutes, importance: value.importance }));
}
__name(validateOrderTasks, "validateOrderTasks");
function parseProviderResponse(raw2, taskIds) {
  let parsed;
  try {
  ...
  const order2 = parsed.order;
  if (order2.length !== taskIds.length || new Set(order2).size !== order2.length || !taskIds.every((id) => order2.includes(id))) return null;
  return { order: order2, reason: parsed.reason };
}
__name(parseProviderResponse, "parseProviderResponse");
async function handleOrder(context, providerOverride) {
  let body;
  try {
  ...
  let raw2;
  try {
    raw2 = await provider(tasks);
  } catch {
    return context.json({ ok: false, error: "PROVIDER_UNAVAILABLE" }, 502);
  }
  const result = parseProviderResponse(raw2, tasks.map((task) => task.id));
...
__name(createOpenAiCompatibleProvider, "createOpenAiCompatibleProvider");
...
app.post("/v1/ai/order", (context) => {
  ...
  const provider = injected ?? (context.env?.LLM_API_KEY ? createOpenAiCompatibleProvider({ apiKey: context.env.LLM_API_KEY, baseUrl: context.env.LLM_BASE_URL ?? "https://api.openai.com/v1", model: context.env.LLM_MODEL ?? "gpt-4o-mini" }) : void 0);
...
// .wrangler/tmp/bundle-rebJ2j/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-rebJ2j/middleware-loader.entry.ts
